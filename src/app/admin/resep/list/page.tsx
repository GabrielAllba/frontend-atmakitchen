...
const List: React.FC = () => {
    ...
    const [filteredData, setFilteredData] = useState<Resep[]>(data);
    ...
    useEffect(() => {
        if (editDetail) {
            const matchingResep = data.find((p) => p.nomor === editDetail.nomor);
    ...
    useEffect(() => {
        if (editResep) {
            const matchingResep = data.find((p) => p.nomor === editResep.nomor);
