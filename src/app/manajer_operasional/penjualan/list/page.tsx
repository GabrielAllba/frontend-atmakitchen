...
const List: React.FC = () => {
    ...
    const fetchSearchResults = async () => {
        ...
        try {
            ...
            axios({
                method: 'get',
                url: `${apiUrl}/laporan_penjualan/${month}/${year}`,
            }).then((response) => {
                ...
                if (data) {
                    for (let index = 0; index < data.length; index++) {
                        const penjualan = data[index];
                        total += penjualan.jumlah_uang;
                    }
                }
                setTotalPrice(total);
                setLoading(false);
                    ...
                    <div className="card bg-primary border pb-8 rounded ">
                        <div className="card-body ">
                            <div className="flex items-center pb-4 w-full justify-between">
                                <p className="text-[#AA2B2B] font-semibold">Data Penjualan</p>
                            </div>
                                ...
                                <table className="table-auto w-full overflow-auto">
                                    <thead>
                                        <tr className="border">
                                            ...
                                            <th className="p-8 border text-start font-semibold">Jumlah Uang</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {filteredData &&
                                            filteredData.map((item) => (
                                                <tr key={item.id} className="border text-[#7D848C]">
                                                    ...
                                                    <td className="p-4 border">{item.produk}</td>
                                                    <td className="p-4 border">{item.kuantitas}</td>
                                        ...
                                        <tr key={'total-price-asdf'} className="border">
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                            <td className="p-4 ">
