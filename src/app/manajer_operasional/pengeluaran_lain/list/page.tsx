...
const List: React.FC = () => {
    ...
    const [filteredData, setFilteredData] = useState<PengeluaranLain[]>([]);
    ...
    const [editPengeluaranLain, setEditPengeluaranLain] = useState<PengeluaranLain>();

    //modal Delete
    const [modalDelete, setModalDelete] = useState<boolean>(false);
    const cancelButtonRef = useRef(null);
    const [deletePengeluaranLain, setDeletePengeluaranLain] = useState<PengeluaranLain>();
    ...
    useEffect(() => {
        const fetchSearchResults = async () => {
            ...
            try {
                const response = await axios.get(apiUrl + '/pengeluaran_lain/search', {
                    params: {
                        query: searchQuery,
                    },
                });
                setFilteredData(response.data.pengeluaran_lain);
            } catch (error) {
                console.error('Error fetching pengeluaran lain:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    // useEffect(() => {
    //     const filtered = data.filter((item) => item.deskripsi.toLowerCase().includes(searchQuery.toLowerCase()));
    //     setFilteredData(filtered);
    // }, [searchQuery]);

    //update Bahan
    const [submitEditPengeluaranLain, setSubmitEditPengeluaranLain] = useState<PengeluaranLain>();

    const handleUpdate = (e: React.FormEvent<HTMLFormElement>, itemId: number) => {
        e.preventDefault();
        console.log(submitEditPengeluaranLain);

        axios({
            method: 'put',
            url: apiUrl + '/pengeluaran_lain/' + itemId,
            data: submitEditPengeluaranLain,
        })
            .then((response) => {
    ...
    const handleDelete = async (id: number) => {
        try {
            const response = await axios.delete(apiUrl + `/pengeluaran_lain/${id}`);
            setSearchQuery('');
            fetchPengeluaranLain();
        } catch (error) {
                    ...
                    <div className="card-body ">
                        ...
                        <div className="overflow-auto">
                            <table className="table-auto w-full overflow-auto">
                                <thead>
                                    <tr className="border">
                                        <th className="p-8 border text-start font-semibold">Deskripsi</th>
                                        <th className="p-8 border text-start font-semibold">Harga</th>
                                        <th className="p-8 border text-start font-semibold">Metode Pembayaran</th>
                                        <th className="p-8 border text-start font-semibold">Tanggal Pengeluaran</th>
                                ...
                                <tbody>
                                    {currentItems.map((item) => (
                                        <tr key={item.id} className="border text-[#7D848C]">
                                            <td className="p-4 border">{item.deskripsi}</td>
                                            <td className="p-4 border">{item.harga}</td>
                                            <td className="p-4 border">{item.metode}</td>
                                            <td className="p-4 border">{item.tanggal_pengeluaran}</td>
                                            ...
                                            <td className="p-4 border">
                                                <div className="flex gap-2">
                                                    <button
                                                        id="openBahan"
                                                        onClick={() => {
                                                            setEditPengeluaranLain(item);
                                                            setSubmitEditPengeluaranLain(item);
                                                ...
                                                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                                    <div className="sm:items-start">
                                                        <form
                                                            ...
                                                            action="#"
                                                            method="PUT"
                                                            onSubmit={(e) => handleUpdate(e, editPengeluaranLain?.id!)}
                                                        >
                                                                    ...
                                                                    <div className="p-4 overflow-auto">
                                                                        <div className="mb-4">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                required
                                                                                value={submitEditPengeluaranLain?.deskripsi}
                                                                                type="text"
                                                                                onChange={(e) => {
                                                                                    const { value } = e.target;

                                                                                    setSubmitEditPengeluaranLain({
                                                                                        ...submitEditPengeluaranLain!,
                                                                                        deskripsi: value,
                                                                                    });
                                                                                }}
                                                                            ></input>
                                                                        ...
                                                                        <div className="mb-4">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                placeholder="harga"
                                                                                required
                                                                                value={submitEditPengeluaranLain?.harga || 0}
                                                                                type="number"
                                                                                onChange={(e) => {
                                                                                    const { value } = e.target;
                                                                        ...
                                                                        <div className="mb-4">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                required
                                                                                value={submitEditPengeluaranLain?.metode || ''}
                                                                                type="text"
                                                                                onChange={(e) => {
                                                                                    const { value } = e.target;
                                                                        ...
                                                                        <div className="mb-4">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                required
                                                                                value={submitEditPengeluaranLain?.tanggal_pengeluaran || ''}
                                                                                type="date"
                                                                                onChange={(e) => {
                                                                                    const { value } = e.target;
                                                    ...
                                                    <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                                        <div className="sm:flex sm:items-start">
                                                            ...
                                                            <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                                                ...
                                                                >
                                                                    Hapus Pengeluaran Lain
                                                                </Dialog.Title>
                                                                <div className="mt-2">
                                                                    <p className="text-sm text-gray-500">
                                                                        Apakah anda yakin ingin menghapus pengeluaran lain ini ?
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                handleDelete(deletePengeluaranLain?.id!);
                                                            }}
                                                        >
                                                            Hapus
                                                        </button>
